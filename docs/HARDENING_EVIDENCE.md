...
Checkpoint: `2dfb0b2`. No deployment or paid OpenAI call is part of this evidence.

| Item | Machine/command-backed criterion | Current evidence |
|---|---|---|
| Workers AI embedding lane | Manifest says BGE base, 768, cosine, one index; five indexes precede insertion | `test/pipeline-manifest.test.ts`, `test/retrieval.test.ts` |
...
Release-candidate command: `npm run check && npm test && XDG_CONFIG_HOME=/tmp/consilium-xdg npm run deploy:check`.
