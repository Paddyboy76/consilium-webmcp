...
const generatedInterpretation=(lens:string)=>({reasoning:`The ${lens} lens weighs the successful protected block against recent counterevidence before recommending a bounded test.`,recommendation:`Use the ${lens} lens to send one bounded pilot invitation now.`,uncertainty:'The canonical history cannot guarantee an external reply.',confidence:.77,confidenceRationale:'The retrieved support is repeated, but counterevidence remains material.',disagreement:`The ${lens} lens preserves a distinct explanation for the shared bounded action.`});
class CloudflareAiFixture {run(model:string,input:{text?:string[]}){if(model==='@cf/meta/llama-3.1-8b-instruct-fast')return Promise.resolve({response:{reports:{'marcus-aurelius':generatedInterpretation('present-duty'),epictetus:generatedInterpretation('controllable-agency'),'sun-tzu':generatedInterpretation('favorable-conditions')},synthesis:{recommendation:'Send one plain pilot invitation before doing more preparation.',resolution:'Three distinct lenses converge on a reversible test.',uncertainty:'The external response remains outside the available evidence.'}}});const texts=input.text??[];return Promise.resolve({data:texts.map(text=>Array.from({length:768},(_,dimension)=>dimension===0?(text.includes('marcus-aurelius')?2:text.includes('epictetus')?3:text.includes('sun-tzu')?4:1):.01))})}}
...
describe('first-request D1 seed',()=>{
  ...
  it('persists a valid structured Workers AI consultation with retrieval-owned canonical evidence',async()=>{
    ...
    const consultation=await worker.fetch(new Request('https://fixture.test/api/council',{method:'POST',headers:{cookie:cookie!,'content-type':'application/json'},body:JSON.stringify({question:'What should I focus on in the next 45 minutes, and why?'})}),env as never),body=await consultation.json<{traceId:string;modelMode:string;fallbackReason:null;retrieval:{provider:string};evidenceProvenance:{ownership:string;interpretation:string;modelCouldSelectIdentifiers:boolean};reports:{advisorId:string;abstained:boolean;recommendation:string;counterevidenceIds:string[];evidence:{id:string;lane:string;relevance:string}[];claims:{personalEvidenceIds:string[];advisorEvidenceIds:string[]}[]}[];validation:{dualGrounded:boolean;persistent_mutation:boolean;evidenceOwnershipAccurate:boolean};decision:{abstained:boolean;validatedReports:unknown[];personalEvidenceIds:string[];advisorEvidenceIds:string[]}}>();
    ...
    expect(body.evidenceProvenance).toMatchObject({ownership:'retrieval-server',interpretation:'workers-ai-generated',modelCouldSelectIdentifiers:false});expect(body.validation).toMatchObject({persistent_mutation:false,evidenceOwnershipAccurate:true});expect(new Set(body.reports.map(report=>report.recommendation)).size).toBe(3);
    for(const report of body.reports){expect(report.abstained).toBe(false);expect(report.claims[0]?.personalEvidenceIds).toHaveLength(1);expect(report.counterevidenceIds).toHaveLength(1);expect(report.claims[0]?.advisorEvidenceIds).toHaveLength(2);expect(report.evidence.every(item=>item.relevance.startsWith('Retrieval-owned'))).toBe(true);for(const id of [...report.claims[0]!.personalEvidenceIds,...report.counterevidenceIds])expect(database.prepare("SELECT COUNT(*) count FROM events WHERE id=? AND user_id='demo-user'").get(id)?.count).toBe(1);for(const id of report.claims[0]!.advisorEvidenceIds)expect(database.prepare("SELECT COUNT(*) count FROM source_chunks sc JOIN council_appointments ca ON ca.pack_id=sc.pack_id AND ca.advisor_id=sc.advisor_id WHERE sc.id=? AND sc.advisor_id=? AND ca.user_id='demo-user' AND ca.ended_at IS NULL").get(id,report.advisorId)?.count).toBe(1)}
    const persisted=database.prepare('SELECT validation_json FROM advisor_reports WHERE consultation_id=?').all(body.traceId) as {validation_json:string}[];expect(persisted).toHaveLength(3);expect(persisted.every(row=>{const value:unknown=JSON.parse(row.validation_json);return typeof value==='object'&&value!==null&&'evidenceOwnership' in value&&value.evidenceOwnership==='retrieval-server'})).toBe(true);expect(database.prepare('SELECT COUNT(*) count FROM actions').get()?.count).toBe(0);expect(database.prepare('SELECT COUNT(*) count FROM proposals').get()?.count).toBe(0);
  });
});
