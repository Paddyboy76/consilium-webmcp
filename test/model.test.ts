...
describe('Workers AI structured council boundary',()=>{
  ...
  it('sends exact selected passages and events as untrusted data without exposing identifiers to generation',()=>{const prompt=JSON.stringify(councilPrompt(bundle()));expect(prompt).toContain('untrusted data');expect(prompt).toContain('Never emit evidence IDs');expect(prompt).toContain('completed the planned pilot action');expect(prompt).toContain('circumscribe the present time');expect(prompt).not.toContain('evt-64-adapt-success');expect(prompt).not.toContain('marcus-b4-03')});
