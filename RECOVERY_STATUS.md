...
- The Pass 22 live consultation trace `trace-8dd68ba2-a0f6-478c-a4a1-53ac7a025ed9` completed in 27,045 ms with six relevant personal memories, exact passages from all three appointed source packs, `validation.valid=true`, `dualGrounded=true`, and `persistent_mutation=false`. Workers AI again produced a non-JSON body (`AI_JSON_SCHEMA_INVALID` after 13,247 ms), so the labelled deterministic fallback supplied the humane evidence-grounded council result without a retry. The trace also exposed and prompted a Pass 23 correction: immediate evidence remains Vectorize-selected, but inferred patterns now always use the complete chronological D1 history.
...
- Council inference uses Cloudflare Workers AI `@cf/meta/llama-3.1-8b-instruct-fast`, under `workers-ai-json-council-v13-single-json-document`. It is on Cloudflare's current JSON Mode support list, and the schema sits directly in `response_format.json_schema` as Cloudflare documents. V12 aligned the provider and Zod contracts; v13 removes formatting variance with zero temperature and isolates at most one JSON document from a single fence or harmless wrapper text. Multiple documents, malformed/truncated JSON, wrong fields, evidence-slot violations, and advisor-scope violations still fail closed. The Worker uses the existing AI binding and no OpenAI API, SDK, separate provider key, or `OPENAI_API_KEY` path.
- The one-call contract now uses temperature 0, strict JSON Schema, 1,600 output tokens, and a 15,000 ms application timeout. There is no retry, cascade, or timeout increase. Server-owned qualified slots, full hydration, semantic validation, deterministic fallback, and every Pass 10–14 reasoning, clinical, provenance, evidence-lane, counterevidence, and no-plan-mutation protection remain intact.
- Object and JSON-string responses still normalize through the identical full schema and hydration path. The normalizer may isolate exactly one object from a single fence or harmless wrapper prose; braces outside a fence, multiple fences, multiple objects, malformed JSON, and truncated JSON fail closed. Zod failures return and audit at most the first eight issue paths and codes, without values or messages. Whole-body parse failure uses only `response_body`/`invalid_json`; other failures expose no arbitrary exception text. Prompts, model prose, private journal text beyond authorized consultation storage, credentials, and secrets are never diagnostic fields.
...
Official rationale: [Workers AI JSON Mode](https://developers.cloudflare.com/workers-ai/features/json-mode/), including the schema request shape and supported-model list.
...
3. Inspect the returned response and `webmcp_calls.result_json`. Verify model `@cf/meta/llama-3.1-8b-instruct-fast`, config `workers-ai-json-council-v13-single-json-document`, `modelMode`, bounded `fallbackReason`, `modelElapsedMs`, advisor-distinct capped lanes, canonical dual grounding, and `persistentMutation=false`.
