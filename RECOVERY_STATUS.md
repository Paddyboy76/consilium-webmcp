...
- Only council inference changes to Cloudflare Workers AI `@cf/openai/gpt-oss-20b`, under `workers-ai-json-council-v9-gpt-oss-wrapper-normalized`. Cloudflare describes it as a Cloudflare-hosted OpenAI open-weight reasoning model intended for lower latency, with a 128,000-token context window and `response_format`. The Workers binding also types `reasoning_effort`, fixed to `low` after the first production v7 call safely timed out at 15 seconds. V9 then normalizes the bounded documented Workers text, Chat Completions, and Responses wrapper shapes after v8 proved the model could return in 5.2 seconds but exposed a nested production envelope. It uses the existing Workers AI binding and no OpenAI API, SDK, separate provider key, or `OPENAI_API_KEY` path. This is the final interactive interpreter choice because it directly targets structured reasoning while retaining the already-proven interactive envelope.
