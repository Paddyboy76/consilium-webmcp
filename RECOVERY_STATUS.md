...
- Council inference uses Cloudflare Workers AI `@cf/meta/llama-3.1-8b-instruct-fast`, under `workers-ai-json-council-v11-fenced-json-normalized`. It is on Cloudflare's current JSON Mode support list, and the schema sits directly in `response_format.json_schema` as Cloudflare documents. V11 permits one exact whole-document JSON fence after v10 returned non-parseable response text; it still rejects any surrounding prose and applies the full Zod, evidence-slot, advisor-scope, and no-mutation gates. The Worker uses the existing AI binding and no OpenAI API, SDK, separate provider key, or `OPENAI_API_KEY` path.
...
3. Inspect the returned response and `webmcp_calls.result_json`. Verify model `@cf/meta/llama-3.1-8b-instruct-fast`, config `workers-ai-json-council-v11-fenced-json-normalized`, `modelMode`, bounded `fallbackReason`, `modelElapsedMs`, advisor-distinct capped lanes, canonical dual grounding, and `persistentMutation=false`.
