...
- Council inference uses Cloudflare Workers AI `@cf/meta/llama-3.1-8b-instruct-fast`, under `workers-ai-json-council-v10-cloudflare-json-mode`. It is on Cloudflare's current JSON Mode support list, and v10 places the schema directly in `response_format.json_schema` as Cloudflare documents. This corrects the earlier OpenAI Chat Completions wrapper assumption: v7 timed out and v8-v9 returned quickly but did not receive the schema at the runtime's expected level. The Worker uses the existing AI binding and no OpenAI API, SDK, separate provider key, or `OPENAI_API_KEY` path.
...
3. Inspect the returned response and `webmcp_calls.result_json`. Verify model `@cf/meta/llama-3.1-8b-instruct-fast`, config `workers-ai-json-council-v10-cloudflare-json-mode`, `modelMode`, bounded `fallbackReason`, `modelElapsedMs`, advisor-distinct capped lanes, canonical dual grounding, and `persistentMutation=false`.
