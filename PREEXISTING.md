...
| `pages/JournalPage.jsx` | Pass 5 restores standalone area/goal-linked journal capture and an area-filtered chronology spanning journal, progress, legacy reflections, structured nightly facts/outcomes/adaptations/directives, with exact IDs. |
...
Pass 5 presents this as Consilium WebMCP with synthetic demo data and a small public-domain corpus. It also restores explicit goal status and makes approved proposals real linked Today goals across Today/Missions/Brief/audit. The exact mapping and legal/runtime exclusions are recorded in `docs/BRAIN2_REUSE_MAP.md` and `docs/FUNCTIONAL_FIDELITY_MATRIX.md`; no private data or copyrighted source text was copied.
