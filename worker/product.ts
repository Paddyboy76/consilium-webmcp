...
async function productState(db:D1Database,session:string){
  ...
  return {areas:areas!.results,missions:missions!.results,progressLogs:logs!.results,journalEntries:journals!.results,reflections:reflections!.results,nightlyReflections:structuredReflections!.results,goalReflections:goalReflections!.results,reflectionFacts:facts!.results,tomorrowDirectives:directives!.results,morningBrief:briefs!.results[0]??null,webmcpCalls:calls!.results,lastWebmcpCall:calls!.results[0]??null,synthetic:true,demoClock:{mode:'frozen-synthetic-evaluation',today:'2026-09-02',historyThrough:'2026-10-06',explanation:'Today is frozen at 2 September for the scripted product journey. The separate synthetic evaluation corpus intentionally includes staged longitudinal observations through 6 October; those are fictional scenario data, not events that happened by Today.'}};
}
...
export async function handleProduct(request:Request,env:ProductBindings,session:string,body:()=>Promise<Record<string,unknown>>):Promise<Response|null>{
  ...
  if(url.pathname==='/api/reflections'&&request.method==='POST'){
    ...
    const synthesis={summary:missed.length?`${missed.length} goal${missed.length===1?' was':'s were'} missed today. Tomorrow starts with the specific change Maya wrote down.`:'Maya reviewed every goal for today and reported no misses. Tomorrow should keep the conditions that helped her focus.',directives:directives.map(item=>item.directive),evidence_ids:[id,...value.goal_reflections.map(goal=>goal.goal_id)],source_citations:[],source_citation_note:'No appointed-book passage is attached: this reflection synthesis is supported by Maya’s submitted reflection and goal reviews only.',uncertainty:'This summary reflects Maya’s own account; it does not claim a philosophical source proves why the day went this way.'};
    await env.DB.batch([
  ...
  if(url.pathname==='/api/briefs/generate'&&request.method==='POST'){
    ...
    const reflectionSections=latestCaar?{intent_and_outcome:latestCaar.q1_today_intent,win_and_friction:latestCaar.q2_top_win,failure_and_mechanism:latestCaar.q3_top_failure,pattern_and_counterevidence:latestCaar.q4_pattern_notice,tomorrow_priority:latestCaar.q5_tomorrow_priority,if_then_plan:latestCaar.q6_if_then_plan}:null;
    const id=uuid('brief'),recommendationId=uuid('recommendation'),generatedAt=new Date().toISOString(),headline=reflectionPriority?`Tomorrow: ${reflectionPriority}`:priorities[0]?`Start with ${priorities[0].title}.`:'No active goal can be selected.',analysis={reflection_id:latest?.id??null,reflection_priority:reflectionPriority,reflection_if_then_plan:reflectionIfThen,reflection_sections:reflectionSections,goal_performance:outcomes.results,pattern,readiness,priorities,earlier_recommendation_ids:previous.results.map(item=>item.id),advisor_citations:[],uncertainty:'Deterministic evidence-grounded fallback; no model-generated causal claims.'},output={id,generatedAt,headline,priorities,evidence,analysis,mode:'deterministic-canonical-fallback',recommendationId};
