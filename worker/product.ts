...
const DEMO_MISSIONS=[
  ...
  ['social-goal','SOC','goal','Call Mum and give her my full attention','I have put this off twice, and I miss hearing what is really going on with her.','today','active',0,'2026-09-03'],
...
const DEMO_UPDATES=[
  ...
  ['SOC','failure',0,'I texted Mum that I would call, then worked too late and missed her evening.'],
  ['FIN','progress',30,'I found two subscriptions I no longer use. I still need to cancel one before it renews.'],
...
const DEMO_REFLECTIONS=[
  ...
  ['SOC','I sent Mum a message and admitted I had lost track of time.','I did not make the call.','Client work ran into the evening and I kept telling myself I would call after one more task.','I did not choose a stopping time for work, so the call was always the thing that moved.','Good intentions are not the same as making time for someone.','Put the call at 6:30 and stop work ten minutes beforehand.','Call Mum at 6:30 tomorrow, with the laptop closed.'],
  ['FIN','I found two charges I had forgotten about.','I did not complete either cancellation.','I checked the last two bank statements and found a meditation app and a stock-photo plan I no longer use.','The cancellation pages were annoying, so I left them open for later.','Finding the waste was useful, but the saving only counts when I cancel it.','Cancel the meditation app while I have the account page open.','Cancel one subscription before checking email tomorrow.'],
...
export async function seedProduct(db:D1Database,session:string){
  ...
  const caar={q1_today_intent:'I finished the client review and drafted a short message to Priya about trying the audit.',q2_top_win:'I avoided sending the message by rewriting the website headline. Tomorrow I will send it before opening the site.',q3_top_failure:'I felt clearest after my lunch walk, when I came back calmer and stopped checking my phone.',q4_pattern_notice:'The client review went well because I left my phone in the other room for forty minutes.',q5_tomorrow_priority:'I let fear of a no turn into unnecessary polishing instead of asking a simple question.',q6_if_then_plan:'Tomorrow my first task is to message Priya. If I want to polish it again, I will send the current draft instead.'};
  ...
  await db.batch([
    ...
    ...CAAR_KEYS.map((key,index)=>db.prepare('INSERT OR IGNORE INTO reflection_facts VALUES(?,?,?,?,?,?,?,?,?)').bind(`${session}-maya-fact-${index+1}`,nightlyId,session,vocGoal,vocArea,['progress','friction','energy','success','misalignment','mission'][index],caar[key],key,at)),
