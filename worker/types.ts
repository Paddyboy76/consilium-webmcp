...
export type TimelineEvent = { id:string; occurredAt:string; type:string; subjectId:string; valence:'positive'|'negative'|'neutral'; magnitude:number; text:string; tags:string[]; area?:string; relationship?:string; goalId?:string; author?:'maya'|'consilium'|'system'; outcome?:string; provenance?:string };
...
export type EvidenceBundle = { question:string; concerns:string[]; goals:string[]; constraints:TimelineEvent[]; history:TimelineEvent[]; priorAdvice:TimelineEvent[]; outcomes:TimelineEvent[]; adaptations:TimelineEvent[]; patterns:Pattern[]; sourceByAdvisor:Record<string,SourceChunk[]>; personalByAdvisor?:Record<string,TimelineEvent[]> };
