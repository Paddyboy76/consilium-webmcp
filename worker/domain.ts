...
export function fixtureReports(bundle:EvidenceBundle):AdvisorReport[]{
  ...
  const available=new Set(bundle.history.map(item=>item.id)),personalId=adaptation?.status==='active' ? [...adaptation.supportingIds].reverse().find(id=>available.has(id))??bundle.history.filter(e=>e.valence==='positive').at(-1)?.id : bundle.history.filter(e=>e.valence==='negative').at(-1)?.id;
