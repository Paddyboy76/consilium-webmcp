...
api('/api/health').then(health=>{const label=document.querySelector('#reasoning-mode'),cloudflare=health.mode==='cloudflare';label.textContent=health.mode==='fixture'?'DETERMINISTIC FIXTURE · NO LIVE MODEL':cloudflare?'CLOUDFLARE RETRIEVAL · DETERMINISTIC COUNCIL':'OPENAI RUNTIME';label.classList.toggle('live',cloudflare)}).catch(()=>{document.querySelector('#reasoning-mode').textContent='MODE UNAVAILABLE'});
